...
import { addToCart, emptyBuyNow, listCartItems, removeFromCart } from '../actions/cartActions';
...
function Cart(props) {
    ...
    return <div className="cart d-flex flex-column">
        {cartItems.length === 0 ? <>
            <Button className="btn-light cart-close-button" onClick={closeCart}><MdClose></MdClose></Button>
            ...
            <div className="d-flex flex-column">
                    ...
                    {cartItems.map(item =>
                        <div key={item.product} className="cart-items">
                            <div className="d-flex align-items-center flex-row">
                                {
                                    item.qty > 1 ? <Button className="ml-3 cart-button" onClick={() => dispatch(addToCart(item.product, (item.qty - 1)))}>
                                        <i className="fa fa-minus"></i></Button>
                                        : <Button className="ml-3 cart-button" onClick={() => removeFromCartHandler(item.product)}>
                                            <i className="fa fa-minus"></i></Button>
                                }
                                <div className="cart-qty">{item.qty}</div>
                                <Button className="cart-button" onClick={() => dispatch(addToCart(item.product, (item.qty + 1)))}>
                                    <i className="fa fa-plus"></i></Button>
                                <div className="ml-2 pt-3 cart-name">{item.name}-{item.netWeight}</div>
                            </div>
                            <div className="d-flex align-items-center flex-row">
                                ...
                                <Button className="ml-1 mr-3 btn-light cart-remove-button" onClick={() => removeFromCartHandler(item.product)}>
                ...
                <div className="cart-checkout-box">
                    ...
                    <Button className="cart-checkout" onClick={checkoutHandler}>Checkout</Button>
                </div>
            </div>}
    </div>
}
